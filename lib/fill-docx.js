...
const ORGANIZATION_ROLE_LABELS = [
  {
    canonical: "教师动作",
    aliases: ["教师动作", "教师活动", "教师具体动作"]
  },
  {
    canonical: "学生任务",
    aliases: ["学生任务", "学生活动", "学生具体任务"]
  },
  {
    canonical: "资源工具",
    aliases: ["资源工具", "使用材料/工具", "材料工具", "教学资源", "使用资源", "材料/工具"]
  },
  {
    canonical: "关键问题/课堂产出",
    aliases: ["关键问题/课堂产出", "关键提问或产出", "问题/产出"]
  },
  {
    canonical: "反馈评价",
    aliases: ["反馈评价", "即时评价", "即时反馈", "评价反馈", "反馈方式", "即时评价或反馈方式"]
  }
];

const ROLE_ALIAS_TO_CANONICAL = new Map(
  ORGANIZATION_ROLE_LABELS.flatMap((group) =>
    group.aliases.map((alias) => [alias, group.canonical])
  )
);

const ROLE_LABEL_PATTERN = ORGANIZATION_ROLE_LABELS
...
function normalizeOrganizationText(value) {
  ...
  if (roleBlocks.length >= 2) {
    return roleBlocks
      .map((block, index) => `${index + 1}、${block.label}：${normalizeNestedOrganizationText(block.content)}`)
      .join("\n");
  }
...
function insertOrganizationRoleBreaks(value) {
  if (!ROLE_LABEL_PATTERN) return value;

  const roleRegex = new RegExp(
    `(?:^|\\s*)(?:\\d{1,2}[、.．)]\\s*)?(?:[•●·*－-]\\s*)?(${ROLE_LABEL_PATTERN})[：:]`,
    "g"
  );

  return value
    .replace(roleRegex, (match, label, offset) => {
      const prefix = offset === 0 ? "" : "\n";
      return `${prefix}${ROLE_ALIAS_TO_CANONICAL.get(label) || label}：`;
    })
    .trim();
}

function collectOrganizationRoleBlocks(value) {
  if (!ROLE_LABEL_PATTERN) return [];

  const roleStartRegex = new RegExp(`^(${ROLE_LABEL_PATTERN})[：:]\\s*(.*)$`);
  const blocks = [];
  let current = null;
  ...
  for (const rawLine of value.split("\n")) {
    ...
    if (match) {
      ...
      current = {
        label: ROLE_ALIAS_TO_CANONICAL.get(match[1]) || match[1],
  ...
  const canonicalOrder = ORGANIZATION_ROLE_LABELS.map((group) => group.canonical);
...
function normalizeNestedOrganizationText(value) {
  const text = normalizeListBreaks(value)
    .replace(/(^|\n)\s*[•●·*－-]\s*/g, "$1")
    .replace(/([“"（(；;。])\s*(\d{1,2})[、.．)]\s*/g, (match, prefix, numberText) => {
      return `${prefix}${toCircledNumber(numberText) || formatNumber(numberText)}`;
    })
    .replace(/(^|\n)\s*[（(](\d{1,2})[)）]\s*/g, (match, prefix, numberText) => {
      return `${prefix}${toCircledNumber(numberText) || formatNumber(numberText)}`;
    })
    .replace(/(^|\n)\s*(\d{1,2})[、.．)]\s*/g, (match, prefix, numberText) => {
      return `${prefix}${toCircledNumber(numberText) || formatNumber(numberText)}`;
    })
    .replace(/(^|\n)\s*([一二三四五六七八九十]{1,3})[、.．)]\s*/g, (match, prefix, numberText) => {
      const number = chineseOrdinalToNumber(numberText);
      return number ? `${prefix}${toCircledNumber(number)}` : match;
    })
    .replace(/(^|\n)\s*([①②③④⑤⑥⑦⑧⑨⑩])\s*/g, "$1$2");
...
function toCircledNumber(value) {
  const number = Number(value);
  if (!Number.isInteger(number) || number < 1 || number > CIRCLED_NUMBERS.length) {
    return "";
  }
