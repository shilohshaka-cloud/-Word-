...
const DEFAULT_TIME_TITLES = ["课程介绍", "导入新课", "教学内容1", "教学内容2", "小结"];
...
function normalizeFieldValue(name, value) {
  ...
  if (TIME_CONTENT_LABELS[name]) {
    return normalizeTimeContentText(text);
  }
  ...
  while (changed) {
    ...
    for (const label of labels) {
      ...
      const pattern = new RegExp(
        `^\\s*(?:(?:\\d+|[一二三四五六七八九十]+)[、.．]\\s*)?${escaped}(?:\\s*[：:：、.．\\-—]+\\s*|\\s*\\n\\s*|\\s*$)`
      );
...
function normalizeTimeContentText(value) {
  return normalizeListBreaks(stripLeadingLabels(value, ALL_TIME_CONTENT_LABELS));
}

function collectTimeAllocations(payload, data) {
  const source = resolveDataSource(payload);
  const rawItems = parseTimeAllocationItems(source.time_allocations);
  if (rawItems.length) {
    return rawItems.map((item, index) => normalizeTimeAllocationItem(item, index)).filter(Boolean);
  }

  return DEFAULT_TIME_TITLES.map((title, index) => {
    const position = index + 1;
    const content = data[`time_${position}_content`];
    const minutes = data[`time_${position}_minutes`];
    return normalizeTimeAllocationItem({ title, content, minutes }, index);
  }).filter(Boolean);
}

function parseTimeAllocationItems(raw) {
  if (!raw) return [];
  ...
  if (typeof raw === "string") {
    ...
    try {
      ...
      if (parsed && Array.isArray(parsed.time_allocations)) return parsed.time_allocations;
  ...
  if (raw && typeof raw === "object") {
    ...
    if (Array.isArray(raw.time_allocations)) return raw.time_allocations;
  }
  return [];
}

function normalizeTimeAllocationItem(item, index) {
  const source = typeof item === "object" && item !== null ? item : { content: item };
  const rawTitle = normalizeValue(
    source.title || source.label || source.name || source.phase || source.section || ""
  );
  const rawContent = normalizeValue(
    source.content || source.text || source.activity || source.description || source.topic || ""
  );
  const rawMinutes = normalizeValue(
    source.minutes || source.minute || source.duration || source.time || source.time_minutes || ""
  );

  const inferredTitle = rawTitle || extractLeadingTimeLabel(rawContent);
  const title = stripTrailingColon(inferredTitle);
  const body = normalizeTimeContentText(rawContent);
  const minutes = cleanMinutes(rawMinutes);

  if (!title && !body && !minutes) return null;

  return {
    title,
    content: title ? `${title}${body ? `：${body}` : ""}` : body,
    minutes
  };
}

function extractLeadingTimeLabel(value) {
  let text = normalizeValue(value);
  text = text.replace(/^\s*(?:\d+|[一二三四五六七八九十]+)[、.．]\s*/, "");
  for (const label of ALL_TIME_CONTENT_LABELS) {
    const escaped = escapeRegExp(label).replace(/\\\//g, "\\s*[/／]\\s*");
...
function replaceTimeAllocationRows(xml, allocations) {
  if (!allocations.length) return xml;

  const rows = [];
  for (let index = 1; index <= 5; index += 1) {
    const row = findRowContaining(xml, `{{time_${index}_content}}`);
    if (!row) return xml;
    rows.push(row);
  }

  const templateRow = rows[0].xml;
  const renderedRows = allocations
    .map((allocation, index) => renderTimeAllocationRow(templateRow, allocation, index))
    .join("");

  return `${xml.slice(0, rows[0].start)}${renderedRows}${xml.slice(rows[rows.length - 1].end)}`;
}
...
function findRowContaining(xml, marker) {
  ...
  const start = findOpeningTagStartBefore(xml, "w:tr", markerIndex);
  const endIndex = xml.indexOf("</w:tr>", markerIndex);
  if (start < 0 || endIndex < 0) return null;

  const end = endIndex + "</w:tr>".length;
  return { start, end, xml: xml.slice(start, end) };
}

function findOpeningTagStartBefore(xml, tagName, beforeIndex) {
...
function renderTimeAllocationRow(templateRow, allocation, index) {
  return templateRow
    .replace(/<w:t>1<\/w:t>/, `<w:t>${index + 1}</w:t>`)
    .replace("课程介绍：{{time_1_content}}", toWordTextXml(allocation.content))
    .replace("{{time_1_minutes}}", toWordTextXml(allocation.minutes));
}
...
function normalizeBodyText(value) {
  return preferPrimaryArabicMarkers(numberBulletLines(normalizeListBreaks(value)));
}

function normalizeStructuredBodyText(value) {
  const withLineBreaks = normalizeListBreaks(value);
  const withBulletLinesNumbered = numberBulletLines(withLineBreaks);
  const withPlainItemsNumbered = enumeratePlainDelimitedItems(withBulletLinesNumbered);
  return preferPrimaryArabicMarkers(withPlainItemsNumbered);
}

function numberBulletLines(value) {
  ...
  return value
    .split("\n")
    .map((line) => {
      const primaryMatch = line.match(/^\s*(\d{1,2})[、.．)]\s*/);
      if (primaryMatch) {
        counter = Math.max(counter, Number(primaryMatch[1]));
        return line;
      }
      ...
      counter += 1;
      return `${bulletMatch[1]}${counter}、${bulletMatch[2]}`;
    })
    .join("\n");
}
...
function insertTeachingResourcesSection(xml, value) {
  ...
  const paragraphStart = findOpeningTagStartBefore(xml, "w:p", markerIndex);
...
async function fillLessonPlanDocx(payload) {
  ...
  const timeAllocations = collectTimeAllocations(payload, data);
  let xml = await documentFile.async("string");

  xml = replaceTimeAllocationRows(xml, timeAllocations);
...
function encodeDownloadToken(payload) {
  ...
  const normalized = {
    ...
    data: {
      ...data,
      time_allocations: collectTimeAllocations(payload, data)
    }
  };
