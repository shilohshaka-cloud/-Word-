...
function normalizeOrganizationText(value) {
  return useCircledMarkers(normalizeListBreaks(value));
}

function useCircledMarkers(value) {
  return value.replace(/(^|\n)\s*(\d{1,2})[.．、]\s*/g, (match, prefix, numberText) => {
    const index = Number(numberText) - 1;
    const marker = CIRCLED_NUMBERS[index] || `${numberText}.`;
...
function toNoBoldWordTextXml(value) {
  const runProperties =
    '<w:rPr><w:rFonts w:ascii="宋体" w:eastAsia="宋体" w:hAnsi="宋体" w:cs="宋体" w:hint="eastAsia"/><w:b w:val="0"/><w:bCs w:val="0"/><w:sz w:val="24"/></w:rPr>';
  const textXml = value
    .split("\n")
    .map((line) => `<w:t>${escapeXml(line)}</w:t>`)
    .join("<w:br/>");

  return `</w:t></w:r><w:r>${runProperties}${textXml}</w:r><w:r><w:t>`;
}

function toWordTextXmlForField(name, value) {
  if (ORGANIZATION_FIELDS.has(name)) return toNoBoldWordTextXml(value);
